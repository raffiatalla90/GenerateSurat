    ...
    <div className="p-1 sm:p-1.5 rounded-[1.75rem] sm:rounded-[2rem] bg-black/[0.04] ring-1 ring-black/5">
      <div className="rounded-[calc(1.75rem-0.25rem)] sm:rounded-[calc(2rem-0.375rem)] bg-white shadow-[inset_0_1px_1px_rgba(255,255,255,0.9),0_12px_40px_rgba(0,0,0,0.06)] overflow-hidden">
                ...
                <div key={item.id} className="group rounded-2xl bg-[#FDFBF7] ring-1 ring-black/5 p-3 md:p-4 hover:bg-white hover:shadow-[0_8px_24px_rgba(0,0,0,0.06)] transition-all duration-700 ease-[cubic-bezier(0.32,0.72,0,1)]">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex flex-col gap-1.5 shrink-0">
                      ...
                      <button onClick={() => { if(confirm(`Hapus riwayat surat ${item.nomorSurat}?\n\nNomor surat ini akan dikembalikan ke urutan yang tersedia.`)) onDelete(item.id); }} className="h-7 px-3 rounded-full bg-white ring-1 ring-black/5 text-xs text-red-600 hover:bg-red-50" title="Hapus riwayat surat">Hapus</button>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div className="px-6 pb-4 text-[11px] text-stone-500 text-center border-t border-black/5 pt-3">
          Maks 50 riwayat • Tersimpan di browser • Nomor surat yang dihapus otomatis dikembalikan
        </div>
      </div>
    </div>
  );
